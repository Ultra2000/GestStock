...
<body>
    ...
    <div class="invoice-info">
        <h3>Fournisseur</h3>
        <p><strong>Nom:</strong> {{ $purchase->supplier->name ?? 'N/A' }}</p>
        <p><strong>Adresse:</strong> {{ $purchase->supplier->address ?? 'N/A' }}</p>
        <p><strong>Téléphone:</strong> {{ $purchase->supplier->phone ?? 'N/A' }}</p>
        <p><strong>Email:</strong> {{ $purchase->supplier->email ?? 'N/A' }}</p>
    </div>

    <div class="invoice-info">
        <p><strong>Numéro de facture:</strong> {{ $purchase->invoice_number }}</p>
    ...
    <div class="total">
        <h3>Total: {{ number_format($purchase->total_amount, 2) }} €</h3>
    </div>
