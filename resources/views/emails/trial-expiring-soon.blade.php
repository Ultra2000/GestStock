...
<head>
    ...
    <title>Période d'évaluation bientôt terminée</title>
    <style>
        ...
        .alert { background: #fef3c7; border: 1px solid #fde68a; border-radius: 10px; padding: 16px 20px; margin-bottom: 24px; }
        .alert p { margin: 0; color: #92400e; font-size: 15px; font-weight: 600; }
...
<div class="container">
    <div class="header">
        <h1>FRECORP ERP</h1>
        <p>Gestion d'entreprise tout-en-un</p>
    </div>
    <div class="body">
        <div class="alert">
            <p>⏰ Plus que {{ $daysLeft }} jour{{ $daysLeft > 1 ? 's' : '' }} d'évaluation gratuite</p>
        </div>

        <p>Bonjour,</p>
        <p>
            Votre période d'évaluation gratuite pour <strong>{{ $company->name }}</strong> se termine dans
            <strong>{{ $daysLeft }} jour{{ $daysLeft > 1 ? 's' : '' }}</strong>.
        </p>
        <p>
            Pour continuer à utiliser FRECORP sans interruption, choisissez votre plan dès maintenant :
        </p>
